...
const SidebarDrawerContext = createContext<TSidebarDrawerContext | undefined>(undefined)
...
export function useSidebarDrawer() {
  ...
  // Return a no-op fallback when not inside the provider (e.g., desktop sidebar)
  if (!context) {
    return {
      isOpen: false,
      open: () => {},
...
export function PageManager({ maxStackSize = 5 }: { maxStackSize?: number }) {
  ...
  const sidebarDrawerContext: TSidebarDrawerContext = {
    isOpen: sidebarDrawerOpen,
    open: () => setSidebarDrawerOpen(true),
    close: () => setSidebarDrawerOpen(false),
    toggle: () => setSidebarDrawerOpen((prev) => !prev)
  }
            ...
            <NotificationProvider>
              ...
              >
                <div className="flex lg:justify-around w-full bg-chrome-background">
                  <div className="sticky top-0 z-40 lg:w-full flex self-start h-[var(--vh)]">
                    <Sidebar />
                  </div>
