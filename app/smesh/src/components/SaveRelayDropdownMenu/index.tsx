...
import { normalizeRelayUrl } from '@/domain/relay/adapters'
...
export default function SaveRelayDropdownMenu({
  ...
  const normalizedUrls = useMemo(() => urls.map((url) => normalizeRelayUrl(url)).filter((url): url is string => url !== null), [urls])
