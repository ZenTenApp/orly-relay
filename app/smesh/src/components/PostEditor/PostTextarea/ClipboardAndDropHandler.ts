...
async function uploadFiles(
  ...
  for (const file of files) {
    ...
    // Use responsive upload for supported images, regular upload for other media
    const uploadPromise = isSupportedImage(file)
      ? mediaUpload.uploadResponsiveImage(file, {
          onProgress: (p) => options.onUploadProgress?.(file, p),
          signal: abortController?.signal
        }).then((result) => {
          // For responsive uploads, use the desktop-sm or original variant URL
          ...
          return { url: primaryVariant?.url ?? result.variants[0]?.url }
        })
