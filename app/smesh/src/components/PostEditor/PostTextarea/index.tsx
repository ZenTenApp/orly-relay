  ...
  (
    ...
    return (
      <Tabs
        ...
        className={cn('space-y-2', fillHeight && 'flex flex-col h-full')}
      >
        <TabsList className="shrink-0">
          <TabsTrigger value="edit">{t('Edit')}</TabsTrigger>
          <TabsTrigger value="preview">{t('Preview')}</TabsTrigger>
        </TabsList>
        <TabsContent value="edit" className={cn(fillHeight && 'flex-1 min-h-0 [&_.tiptap]:h-full')}>
          <EditorContent className={cn('tiptap', fillHeight && 'h-full')} editor={editor} />
        </TabsContent>
        <TabsContent
          value="preview"
          className={cn(fillHeight && 'flex-1 min-h-0 overflow-auto')}
          onClick={() => {
            setTabValue('edit')
            editor.commands.focus()
          }}
        >
          <Preview content={text} className={cn(className, fillHeight && 'h-full')} />
        </TabsContent>
      </Tabs>
    )
  }
