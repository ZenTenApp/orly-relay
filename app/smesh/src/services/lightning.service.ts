...
const OFFICIAL_PUBKEYS = [SMESH_PUBKEY]
...
class LightningService {
  ...
  /**
   * Initialize bitcoin-connect. Call this AFTER setting up onConnected/onDisconnected listeners
   * to avoid race conditions with auto-reconnect.
   */
  initBitcoinConnect() {
  ...
  async zap(
    ...
    const [profile, receiptRelayList, senderRelayList] = await Promise.all([
      ...
      sender
        ...
        : Promise.resolve({ read: client.currentRelays, write: client.currentRelays })
    ])
    if (!profile) {
    ...
    return new Promise((resolve) => {
      ...
      if (verify) {
        ...
        subCloser = client.subscribe(
          senderRelayList.write.concat(client.currentRelays).slice(0, 4),
          filter,
          {
            onevent: (evt) => {
  ...
  async fetchRecentSupporters() {
    ...
    const relayList = await client.fetchRelayList(SMESH_PUBKEY)
