...
class ThreadService {
  ...
  async subscribe(stuff: NostrEvent | string, limit = 100) {
    ...
    const _subscribe = async () => {
      ...
      // Phase 3: Improved relay selection for thread loading
      // 1. Add relay hint from rootInfo (for replaceable events)
      if (rootInfo.type === 'A' && rootInfo.relay) {
        relaySet.add(rootInfo.relay)
      }

      // 2. Add relays where we saw this event
      if (event) {
        const seenOn = client.getSeenEventRelayUrls(event.id)
        seenOn.forEach((url) => relaySet.add(url))
      }

      // 3. Add author's relays
      if (rootPubkey) {
        const relayList = await client.fetchRelayList(rootPubkey)
        relayList.read.slice(0, 4).forEach((url) => relaySet.add(url))
      }

      // 4. Fall back to current user's relays if needed
      if (relaySet.size === 0) {
        client.currentRelays.forEach((url) => relaySet.add(url))
      }

      let relayUrls = Array.from(relaySet).slice(0, 8)
  ...
  private async tryGraphQueryThread(eventId: string): Promise<boolean> {
    try {
      // Use current user's relays for graph queries
