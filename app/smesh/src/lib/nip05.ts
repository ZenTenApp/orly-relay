...
function proxyUrl(url: string): string {
  ...
  if (proxyServer) {
    return `${proxyServer}/sites/${encodeURIComponent(url)}`
  }
  return url
}
...
async function _verifyNip05(nip05: string, pubkey: string): Promise<TVerifyNip05Result> {
  ...
  try {
    const res = await fetch(proxyUrl(getWellKnownNip05Url(nip05Domain, nip05Name)))
...
export async function fetchPubkeysFromDomain(domain: string): Promise<string[]> {
  try {
    const res = await fetch(proxyUrl(getWellKnownNip05Url(domain)))
    const json = await res.json()
    const pubkeySet = new Set<string>()
    return Object.values(json.names || {}).filter((pubkey) => {
      if (typeof pubkey !== 'string' || !Pubkey.isValidHex(pubkey)) {
        return false
      }
