/**
 * Infrastructure utilities for Nostr event parsing and manipulation.
 *
 * These are infrastructure-level helpers for working with raw Nostr events.
 * They handle event parsing, tag extraction, and event comparison.
 *
 * Note: For domain-level event handling, consider using domain entities:
 *   import { Note, EventId } from '@/domain'
 *
 * The Note entity provides domain-focused methods like:
 *   - note.isReply, note.isRoot
 *   - note.mentions, note.references
 *   - note.hashtags, note.contentWarning
 */
