...
export function getRelayListFromEvent(
  ...
  if (!event) {
    return { write: [], read: [], originalRelays: [] }
  }
...
export function getProfileFromEvent(event: Event) {
  try {
    ...
    const pk = Pubkey.tryFromString(event.pubkey)
    return {
      pubkey: event.pubkey,
      npub: pk?.npub ?? '',
      banner: profileObj.banner,
      avatar: profileObj.picture,
      username: username || (pk?.formatNpub(12) ?? event.pubkey.slice(0, 8)),
    ...
    return {
      pubkey: event.pubkey,
      npub: pk?.npub ?? '',
      username: pk?.formatNpub(12) ?? event.pubkey.slice(0, 8)
    }
  }
}
...
export function getFollowPackInfoFromEvent(event: Event) {
  ...
  event.tags.forEach(([tagName, tagValue]) => {
    if (tagName === 'title') {
      ...
      image = tagValue
    } else if (tagName === 'p' && Pubkey.isValidHex(tagValue)) {
