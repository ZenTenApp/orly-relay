...
/**
 * Get Neo4j configuration status (no auth required)
 * @returns {Promise<object>} Neo4j config status { db_type: string }
 */
export async function fetchNeo4jConfig() {
    try {
        const response = await fetch(`${getApiBase()}/api/neo4j/config`);
        if (response.ok) {
            return await response.json();
        }
    } catch (error) {
    ...
    return { db_type: "badger" };
}

/**
 * Get Neo4j bolt+s configuration (owner auth required)
 * @param {object} signer - The signer instance
 * @param {string} pubkey - User's pubkey
 * @returns {Promise<object>} Bolt config status
 */
export async function fetchNeo4jBoltConfig(signer, pubkey) {
    const url = `${getApiBase()}/api/neo4j/bolt`;
    const authHeader = await createNIP98Auth(signer, pubkey, "GET", url);
    const response = await fetch(url, {
        headers: authHeader ? { Authorization: `Nostr ${authHeader}` } : {},
    });
    if (!response.ok) {
        const error = await response.text();
        throw new Error(error || `Failed to get Neo4j bolt config: ${response.statusText}`);
    }
    return await response.json();
}

/**
 * Toggle bolt+s on or off in neo4j.conf and restart Neo4j
 * @param {object} signer - The signer instance
 * @param {string} pubkey - User's pubkey
 * @param {boolean} enabled - Whether to enable or disable bolt+s
 * @returns {Promise<object>} Toggle result { success, message, enabled }
 */
export async function toggleNeo4jBolt(signer, pubkey, enabled) {
    const url = `${getApiBase()}/api/neo4j/bolt/toggle`;
    const authHeader = await createNIP98Auth(signer, pubkey, "POST", url);
    const response = await fetch(url, {
        method: "POST",
        headers: {
