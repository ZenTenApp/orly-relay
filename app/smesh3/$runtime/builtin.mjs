...
// Returns null (not undefined) for missing keys — Go zero value semantics.
// null == false, null == 0, null == "" are all false in JS,
// but null is handled by nil checks throughout the runtime.
export function mapLookup(m, key) {
  if (m === null || m === undefined) return { value: null, ok: false };
  if (m instanceof Map) {
