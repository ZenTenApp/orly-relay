		...
		t.Run(
			tc.name, func(t *testing.T) {
				...
				if len(valueField) != tc.internalLen {
					t.Errorf(
						"Expected internal value length %d, got %d",
						tc.internalLen, len(valueField),
					)
				}
				...
				if string(marshaled) != tc.json {
					t.Errorf(
						"Marshaled JSON doesn't match original.\nExpected: %s\nGot: %s",
						tc.json, string(marshaled),
					)
				}
			},
		)
...
func TestValueHexMethod(t *testing.T) {
	...
	if string(hexVal) != expectedHex {
		t.Errorf(
			"ValueHex returned wrong value.\nExpected: %s\nGot: %s",
			expectedHex, string(hexVal),
		)
	}
}
...
func TestBinaryEncodingSavesSpace(t *testing.T) {
	...
	if optSize >= nonOptSize {
		t.Errorf(
			"Binary encoding should save space. Non-opt: %d bytes, Opt: %d bytes",
			nonOptSize, optSize,
		)
	}
	...
	if actualSavings != expectedSavings {
		t.Errorf(
			"Expected to save %d bytes, actually saved %d bytes",
			expectedSavings, actualSavings,
		)
	}

	t.Logf(
		"Space savings: %d bytes per e/p tag value (%.1f%% reduction)",
