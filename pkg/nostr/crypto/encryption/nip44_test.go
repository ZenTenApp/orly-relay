...
func assertCryptPriv(
	...
	actual, err = nip44.Encrypt(
		k1, []byte(plaintext), &nip44.EncryptOptions{Salt: s},
	)
...
func assertCryptPub(
	t *testing.T, sk1 string, pub2 string, conversationKey string, salt string,
	plaintext string, expected string,
) {
	var (
		k1        []byte
...
func assertDecryptFail(
	t *testing.T, conversationKey string, plaintext string, ciphertext string,
	msg string,
) {
	var (
		k1  []byte
...
func assertConversationKeyFail(
	t *testing.T, sk1 string, pub2 string, msg string,
) {
	var (
		sk1Decoded  []byte
		pub2Decoded []byte
	...
	if ok = assert.NoErrorf(
		t, err, "hex decode failed for pub2: %v", err,
	); !ok {
		return
	}
...
func assertConversationKeyGeneration(
	t *testing.T, sendPrivkey []byte, recvPubkey []byte, conversationKey string,
) bool {
	var (
	...
	actualConversationKey, err = nip44.GenerateConversationKey(
		sendPrivkey, recvPubkey,
	)
	if ok = assert.NoErrorf(
...
func assertConversationKeyGenerationSec(
	t *testing.T, sk1 string, sk2 string, conversationKey string,
) bool {
	var (
		sk1Decoded  []byte
		pub2Decoded []byte
	...
	return assertConversationKeyGeneration(
		t, sk1Decoded, pub2Decoded, conversationKey,
	)
}

func assertConversationKeyGenerationPub(
	t *testing.T, sk1 string, pub2 string, conversationKey string,
) bool {
	var (
		sk1Decoded  []byte
		pub2Decoded []byte
...
func assertMessageKeyGeneration(
	t *testing.T, conversationKey string, salt string, chachaKey string,
	chachaSalt string, hmacKey string,
) bool {
	var (
		convKey             []byte
		convSalt            []byte
	...
	if ok = assert.NoErrorf(
		t, err, "hex decode failed for chacha key: %v", err,
	); !ok {
		return false
	}
	expectedChaChaNonce, err = hex.DecodeString(chachaSalt)
	if ok = assert.NoErrorf(
		t, err, "hex decode failed for chacha nonce: %v", err,
	); !ok {
		return false
	}
	...
	actualChaChaKey, actualChaChaNonce, actualHmacKey, err = nip44.MessageKeys(
		convKey, convSalt,
	)
	if ok = assert.NoErrorf(
	...
	if ok = assert.Equalf(
		t, expectedChaChaKey, actualChaChaKey, "wrong chacha key",
	); !ok {
		return false
	}
	if ok = assert.Equalf(
		t, expectedChaChaNonce, actualChaChaNonce, "wrong chacha nonce",
	); !ok {
		return false
	}
	if ok = assert.Equalf(
...
func assertCryptLong(
	t *testing.T, conversationKey string, salt string, pattern string,
	repeat int, plaintextSha256 string, payloadSha256 string,
) {
	var (
		convKey               []byte
		convSalt              []byte
	...
	actualPayload, err = nip44.Encrypt(
		convKey, []byte(plaintext), &nip44.EncryptOptions{Salt: convSalt},
	)
