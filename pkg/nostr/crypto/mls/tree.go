...
const (
	...
	// ExtensionTypeLastResort marks a KeyPackage as reusable for multiple
	// Welcome messages. Required by Marmot (MIP-00).
	ExtensionTypeLastResort extensionType = 0x000a

	// ExtensionTypeNostrGroupData carries Nostr group metadata (group ID,
	// name, admins, relays). Required by Marmot (MIP-01).
	ExtensionTypeNostrGroupData extensionType = 0xf2ee
)

// Extension holds a TLS-serialized MLS extension (type + opaque data).
...
// NewExtension creates an extension with the given type and data.
func NewExtension(t extensionType, data []byte) extension {
	return extension{extensionType: t, extensionData: data}
}

// ExtensionType is exported for use by the Marmot SDK.
