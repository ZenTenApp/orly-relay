...
func (group *Group) ExporterSecret() ([]byte, error) {
	return group.groupContext.cipherSuite.deriveSecret(
		group.epochSecret, secretLabelExporter,
	)
}

// GroupContextExtensions returns the extensions from the group context.
// Use this to extract application-specific data like NostrGroupData (0xf2ee).
func (group *Group) GroupContextExtensions() []extension {
	return group.groupContext.extensions
}

// FindGroupContextExtension returns the data for the extension with the given
// type, or nil if not found.
func (group *Group) FindGroupContextExtension(t extensionType) []byte {
...
// DeriveExporter exports keying material from the group's exporter secret
// using the MLS exporter derivation (RFC 9420 Section 8).
func (group *Group) DeriveExporter(label, context []byte, length uint16) ([]byte, error) {
	exporterSecret, err := group.ExporterSecret()
	if err != nil {
		return nil, err
	}
	return deriveExporter(group.groupContext.cipherSuite, exporterSecret, label, context, length)
}

// GroupOptions configures group creation.
type GroupOptions struct {
	// Extensions are included in the group context. For Marmot, this
	// should include a NostrGroupData extension (0xf2ee).
...
func CreateGroup(groupID GroupID, keyPairPkg *KeyPairPackage) (*Group, error) {
	return CreateGroupWithOptions(groupID, keyPairPkg, nil)
}

// CreateGroupWithOptions creates a new group with custom group context extensions.
func CreateGroupWithOptions(groupID GroupID, keyPairPkg *KeyPairPackage, opts *GroupOptions) (*Group, error) {
	...
	var ctxExts []extension
	if opts != nil {
		ctxExts = opts.Extensions
	}
...
// is returned.
func (group *Group) UnmarshalAndProcessMessage(raw []byte) (plaintext []byte, selfSent bool, err error) {
	var msg mlsMessage
	if err := unmarshal([]byte(raw), &msg); err != nil {
		return nil, false, fmt.Errorf("failed to unmarshal MLS message: %v", err)
	}

	switch msg.wireFormat {
	case wireFormatMLSPublicMessage:
		return nil, false, group.processPublicMessage(msg.publicMessage)
	case wireFormatMLSPrivateMessage:
		return group.processPrivateMessage(msg.privateMessage)
	default:
		// TODO: support other wire formats
		return nil, false, fmt.Errorf("mls: unsupported wire format: %v", msg.wireFormat)
	}
}
...
func (group *Group) processPrivateMessage(privMsg *privateMessage) ([]byte, bool, error) {
	cs := group.groupContext.cipherSuite

	if !privMsg.groupID.Equal(group.groupContext.groupID) {
		return nil, false, fmt.Errorf("mls: message group ID mismatch")
	}
	if privMsg.epoch != group.groupContext.epoch {
		return nil, false, fmt.Errorf("mls: epoch mismatch: got %v, want %v", privMsg.epoch, group.groupContext.epoch)
	}

	senderDataSecret, err := cs.deriveSecret(group.epochSecret, secretLabelSenderData)
	if err != nil {
		return nil, false, fmt.Errorf("failed to derive sender data secret: %v", err)
	}

	senderData, err := privMsg.decryptSenderData(cs, senderDataSecret)
	if err != nil {
		return nil, false, fmt.Errorf("failed to decrypt sender data: %v", err)
	}

	encryptionSecret, err := cs.deriveSecret(group.epochSecret, secretLabelEncryption)
	if err != nil {
		return nil, false, fmt.Errorf("failed to derive encryption secret: %v", err)
	}

	secretTree, err := deriveSecretTree(cs, group.tree.numLeaves(), encryptionSecret)
	if err != nil {
		return nil, false, fmt.Errorf("failed to erive secret tree: %v", err)
	}
	...
	if err != nil {
		return nil, false, fmt.Errorf("failed to derive secret ratchet tree root: %v", err)
	}
	...
	for secret.generation != senderData.generation {
		...
		if err != nil {
			return nil, false, fmt.Errorf("failed to derive next ratchet secret: %v", err)
		}
	}

	privContent, err := privMsg.decryptContent(cs, secret, senderData.reuseGuard)
	if err != nil {
		return nil, false, fmt.Errorf("failed to decrypt private message content: %v", err)
	}

	signerNode := group.tree.getLeaf(senderData.leafIndex)
	if signerNode == nil {
		return nil, false, fmt.Errorf("mls: signer node is blank")
	}
	...
	if !authContent.verifySignature(signerNode.signatureKey, &group.groupContext) {
		return nil, false, fmt.Errorf("failed to verify private message content signature: %v", err)
	}

	selfSent := senderData.leafIndex == group.myLeafIndex

	switch authContent.content.contentType {
	case contentTypeProposal:
		return nil, false, group.processProposal(authContent)
	case contentTypeCommit:
		return nil, false, group.processCommit(authContent, nil, nil, nil)
	case contentTypeApplication:
		return authContent.content.applicationData, selfSent, nil
	default:
		// TODO: support other content types
		return nil, false, fmt.Errorf("mls: unsupported content type: %v", authContent.content.contentType)
	}
}
